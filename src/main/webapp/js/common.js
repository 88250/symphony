...
var Util = {
  ...
  initCodeMirror: function () {
      ...
      success: function (result) {
        ...
        for (var i = 0; i < emojis.length; i++) {
          ...
          emojiAutocompleteHints.push({
            ...
            '&nbsp;<img style="width: 16px" src="' + Label.staticServePath +
            '/emoji/graphics/' + text + '.png"></span>',
            text: text + ': ',
          })
        }
          ...
          for (var i = 0; i < emojis.length; i++) {
            ...
            if (Util.startsWith(text, input)) {
              autocompleteHints.push({
                ...
                Label.staticServePath + '/emoji/graphics/' + text + '.png"> ' +
        ...
        $.ajax({
          ...
          success: function (result) {
            ...
            for (var i = 0; i < result.userNames.length; i++) {
              ...
              autocompleteHints.push({
                displayText: '<span style=\'font-size: 1rem;line-height:22px\'><img style=\'width: 1rem;height: 1rem;margin:3px 0;float:left\' src=\'' +
                avatar
                + '\'> ' + name + '</span>',
                text: name + ' ',
              })
            ...
            if ('comment' === cm['for'] &&
              ('@participants'.indexOf(tok.string) > -1)) {
              autocompleteHints.push({
    ...
    CodeMirror.commands.endAudioRecord = function (cm) {
      ...
      var cursor = cm.getCursor()
      cm.replaceRange(Label.uploadingLabel,
        CodeMirror.Pos(cursor.line, cursor.ch -
          Label.audioRecordingLabel.length), cursor)
        ...
        if ('' !== Label.qiniuUploadToken) {
          ...
          $.ajax({
            ...
            success: function (data) {
              var cursor = cm.getCursor()
              cm.replaceRange('<audio controls="controls" src="' +
                Label.qiniuDomain + '/' + key + '"></audio>\n\n',
                CodeMirror.Pos(cursor.line, cursor.ch -
                  Label.uploadingLabel.length), cursor)
            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
              Util.alert('Error: ' + errorThrown)
              var cursor = cm.getCursor()
              cm.replaceRange('', CodeMirror.Pos(cursor.line, cursor.ch -
                Label.uploadingLabel.length), cursor)
            },
          })
          ...
          $.ajax({
            ...
            success: function (data) {
              var cursor = cm.getCursor()
              cm.replaceRange('<audio controls="controls" src="' + data.key +
                '"></audio>\n\n',
                CodeMirror.Pos(cursor.line, cursor.ch -
                  Label.uploadingLabel.length), cursor)
            },
  ...
  init: function (isLoggedIn) {
    ...
    this._initBreezemoon($('#breezemoonPostBtn'), $('#breezemoonInput'), isLoggedIn)
          ...
          if (window.File && window.FileReader && window.FileList &&
            ...
            reader.onload = function (evt) {
              ...
              if (isImg && evt.target.result.byteLength > obj.imgMaxSize) {
                Util.alert('This image is too large (max ' + obj.imgMaxSize /
                  1024 / 1024 + 'M)')
              ...
              if (!isImg && evt.target.result.byteLength > obj.fileMaxSize) {
                Util.alert('This file is too large (max ' + obj.fileMaxSize /
                  1024 / 1024 + 'M)')
          ...
          if (data.result.code === 1) {
            ...
            if (obj.editor.replaceRange) {
              ...
              obj.editor.replaceRange('[' + filename + '](Error) \n\n',
                CodeMirror.Pos(cursor.line, cursor.ch -
                  obj.uploadingLabel.length), cursor)
            } else {
              obj.editor.$it.val(obj.editor.$it.val() + '![' + filename +
                '](Error) \n\n')
    ...
    $('#' + obj.id).fileupload({
      ...
      submit: function (e, data) {
        ...
        if (obj.editor.replaceRange) {
          ...
          if (isImg) {
            obj.editor.replaceRange('![' + filename + '](' + obj.qiniuDomain +
              '/' + qiniuKey + ') \n\n',
              CodeMirror.Pos(cursor.line, cursor.ch -
                obj.uploadingLabel.length), cursor)
          } else {
          ...
          obj.editor.$it.val('![' + filename + '](' + obj.qiniuDomain + '/' +
            qiniuKey + ') \n\n')
          $('#' + obj.id + ' input').prop('disabled', false)
        }
        fileIndex--
      ...
      fail: function (e, data) {
        ...
        if (obj.editor.replaceRange) {
          var cursor = obj.editor.getCursor()
          obj.editor.replaceRange('',
            CodeMirror.Pos(cursor.line, cursor.ch - obj.uploadingLabel.length),
            cursor)
        } else {
          $('#' + obj.id + ' input').prop('disabled', false)
        }
